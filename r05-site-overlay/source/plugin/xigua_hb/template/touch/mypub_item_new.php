...
<div class="flit li<!--{if $v[wancheng]}--> op6<!--{/if}--> <!--{if $v[dig_on] && $v[zdcolor]}-->digclr" style="background-color:{$v[zdcolor]}<!--{/if}-->" id="li_$v[id]">
<div class="po-avt-wrap">
<a href="javascript:;"><img onclick="hb_jump('$SCRITPTNAME?id=xigua_hb&ac=member&uid=$v[uid]');" class="po-avt" style="width:44px;height:44px;display:block;border:2px solid #edf3fa;border-radius:50%;object-fit:cover;" src="{avatar($v[uid], 'middle', true)}"></a>
</div>
<div class="po-cmt">
<div class="po-hd cl <!--{if $v[wancheng]}-->wxexpired1<!--{/if}-->">
            <!--{if !$_GET[hb]}-->
            <!--{if !$v[pay_status]&& $_GET['is_my']}-->
            <a class="abs b-color8 mod-feed-tag main_color" style="top:1px" href="$SCRITPTNAME?id=xigua_hb&ac=pay&catid=$v[catid]&pubid=$v[id]"><!--{if IS_ADMINID}-->{lang xigua_hb:wei}<!--{else}-->{lang xigua_hb:pay}<!--{/if}--></a>
            <!--{else}-->
            <!--{if $v[display]}-->
            ...
            <a class="abs <!--{if $_GET[pinstyle]}-->pstyle1<!--{/if}-->" href="$SCRITPTNAME?id=xigua_hb&ac=cat&cat_id=$v[catid]">$cats[$v[catid]][name]</a>
            <!--{/if}-->
            <!--{/if}-->
    <!--{elseif !$v[display] &&$v[reason]}-->
    <a style="max-width: 7rem;z-index: 501;background: #fff;text-align: left;" class="abs b-color8 mod-feed-tag main_color">$v[reason]</a>
                <!--{else}-->
                <a class="abs b-color8 mod-feed-tag main_color">{lang xigua_hb:daishen}</a>
                <!--{/if}-->
            <!--{/if}-->
            <!--{else}-->
...
<!--{if $v['refresh_times']<$v['jiangemax'] && $v['jiange']>0}--><div class="mod-lv is-green">&#33258;&#21160;&#21047;&#26032;</div><!--{/if}-->
<!--{if $v[ipaddr]}--></div><!--{/if}-->
            </div>
            <div class="post cl">
                ...
                <!--{/if}-->
                <div id="view_jump_$v[id]" class="view_jump mod-feed-text cl is-three" data-id="$v[id]" data-stid="$v[stid]"><!--{if $config[listcattype]==2}--><a class="bftag b-color0 main_color" href="$SCRITPTNAME?id=xigua_hb&ac=cat&cat_id=$v[catid]">$cats[$v[catid]][name]</a><!--{/if}-->
<!--{if $config[customfisrt]!=2}--><span class="">{echo nl2br(cutstr(str_replace(array("\n\n","\r\r", "\n\r\n\r"), '', trim(strip_tags($v[description]))), 200));}</span><!--{/if}-->
                    <!--{eval $distvar = '';}-->
                    <!--{loop $v[vars] $vr}-->
                    ...
                    <!--{/if}-->
                    <!--{/loop}-->
<!--{if $config[customfisrt]==2}--><span class="">{echo cutstr(nl2br(str_replace(array("\n\n","\r\r", "\n\r\n\r"), '', trim(strip_tags($v[description])))), 200);}</span><!--{/if}-->
...
$img = $GLOBALS_QN ? $img.'?imageView2/1/q/80' : ( $GLOBALS_OSS ? $img.'?x-oss-process=image/resize,h_640,w_640/format,jpg/quality,q_80' : $img);
                    ...
                    <!--{if $showzhang}--><em class="num">$v[img_count]{lang xigua_hb:zhang}</em><!--{/if}-->
                    </span>
                    <!--{/loop}-->
                </div>
                <!--{if $v[goodname]}-->
                ...
                <a class="hs_inner_loc da" target="_blank" href="$SCRITPTNAME?id=xigua_hs&ac=view&shid={$v[sh][shid]}"><i class="iconfont icon-coordinates_fill f14 "></i>{$v[sh][addr]}</a>
                <!--{elseif $distvar}-->
                <a class="hs_inner_loc da" style="display:block" target="_blank" href="$SCRITPTNAME?id=xigua_hb&ac=view&pubid=$v[id]"><i class="iconfont icon-coordinates_fill f14 "></i>{$distvar[value][0]} <!--{if $v[distance]}--><span class="diskm">{lang xigua_hb:juwo}{$v[distance]}</span><!--{/if}--></a>
                <!--{/if}-->
            </div>
            <!--{if !$_GET[hb]}-->
            <div class="cl pr">
                <p class="time">
                    ...
                    <span><em>$v[shares]</em>{lang xigua_hb:fenxiangdot}</span>
                    <span>$v[time_u]<!--{if $v[refresh_times]}-->{lang xigua_hb:shuaxin}<!--{else}-->{lang xigua_hb:fabu0}<!--{/if}--></span>
<!--{if $_G[uid] == $v[uid]||IS_ADMINID }--><span>{lang xigua_hb:lastts}:{echo date('Y-m-d', $v[endts])}</span><!--{/if}-->
                </p>

                <!--{if ($_G[uid] == $v[uid]||IS_ADMINID)  }-->
                <a <!--{if $_GET[stat]!='endts'}-->style="display:none"<!--{/if}--> class="a c_opt" href="javascript:;" id="pubitem_$v[id]" data-id="$v[id]" data-uid="$v[uid]" data-wc="{$v[wancheng]}" <!--{if $v[display]}-->data-canzd="1"<!--{/if}--> <!--{if (!$v[hb_num]||$v[hb_num]==$v[hb_sendnum])&&$v[display]&&$config[red]}-->data-canhb="1"<!--{/if}--> <!--{if !$v[pay_status]}-->data-catid="$v[catid]"<!--{/if}--> onclick="return showansi(this);"><!--{if IS_ADMINID}-->{lang xigua_hb:guanli0}<!--{else}-->{lang xigua_hb:guanli}<!--{/if}--></a>
                <!--{/if}-->
                <!--{if $_GET[stat]!='endts'}--><i class="c-icon iconfont icon-qunawanhuifu" id="c_icon_$v[id]" <!--{if $v[dig_on] && $v[zdcolor]}-->style="background-color:{$v[zdcolor]}"<!--{/if}-->></i><!--{/if}-->
...
<!--{/if}-->
<!--{if ($_G[uid] == $v[uid]||IS_ADMINID) && ($v[display]||$_GET[is_admin]) && $_GET[stat]!='endts' }-->
<div class="po-act">
