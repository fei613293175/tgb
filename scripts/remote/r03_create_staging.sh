...
SOURCE_STAGING_BASE="/www/staging/tg-h5-ui-r02"
...
STAGE_MAIN_USER="tgb_r03_main"
STAGE_UC_DB="tgb_stage_r03_uc"
STAGE_UC_USER="tgb_r03_uc"
...
abort_if_exists() {
    if [ -e "$1" ]; then
        printf '[R03] ABORT existing path: %s\n' "$1" >&2
        exit 20
    fi
}

if [ "$(id -u)" -ne 0 ]; then
    printf '[R03] ABORT: root is required\n' >&2
    exit 21
fi
...
[ "${SOURCE_POST_CODE}" = "405" ] || {
    printf '[R03] ABORT: R02 source staging write guard is %s\n' \
        "${SOURCE_POST_CODE}" >&2
    exit 30
}
...
if ss -ltn | awk '{print $4}' | grep -Eq "(^|:)${LOOPBACK_PORT}$"; then
    printf '[R03] ABORT busy loopback port: %s\n' "${LOOPBACK_PORT}" >&2
    exit 29
fi
...
ROOT_CNF="$(mktemp /tmp/tgb-r03-root.XXXXXX.cnf)"
cleanup() {
    rm -f "${ROOT_CNF}"
}
trap cleanup EXIT
...
log "resolve R02 source database identifiers without printing credentials"
MAIN_SOURCE_DB="$(php -r 'include "/www/staging/tg-h5-ui-r02/site/config/config_global.php"; echo $_config["db"][1]["dbname"];')"
MAIN_TABLE_PREFIX="$(php -r 'include "/www/staging/tg-h5-ui-r02/site/config/config_global.php"; echo $_config["db"][1]["tablepre"];')"
UC_SOURCE_DB="$(php -r 'include "/www/staging/tg-h5-ui-r02/site/uc_server/data/config.inc.php"; echo UC_DBNAME;')"
UC_TABLE_PREFIX="$(php -r 'include "/www/staging/tg-h5-ui-r02/site/uc_server/data/config.inc.php"; echo UC_DBTABLEPRE;')"

[ "${MAIN_SOURCE_DB}" = "tgb_stage_r02_main" ] || {
    printf '[R03] ABORT: unexpected R02 main source database\n' >&2
    exit 31
}
[ "${UC_SOURCE_DB}" = "tgb_stage_r02_uc" ] || {
    printf '[R03] ABORT: unexpected R02 UCenter source database\n' >&2
    exit 32
}

case "${MAIN_SOURCE_DB}:${MAIN_TABLE_PREFIX}:${UC_SOURCE_DB}:${UC_TABLE_PREFIX}" in
    *[!A-Za-z0-9_:]*)
        printf '[R03] ABORT: database identifier contains unsupported characters\n' >&2
        exit 22
        ;;
esac
...
for db_name in "${STAGE_MAIN_DB}" "${STAGE_UC_DB}"; do
    if mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
        ...
        grep -q .; then
        printf '[R03] ABORT existing database: %s\n' "${db_name}" >&2
        exit 23
    fi
done
...
for db_user in "${STAGE_MAIN_USER}" "${STAGE_UC_USER}"; do
    if mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
        ...
        grep -q .; then
        printf '[R03] ABORT existing database user: %s\n' "${db_user}" >&2
        exit 24
    fi
done
...
log "create code snapshot from closed R02 staging"
...
if old in text:
    ...
    raise SystemExit("R02 staging H5 entry is neither local nor expected production URL")
...
for forbidden in \
    ...
    if [ -e "${STAGING_SITE}/${forbidden}" ]; then
        printf '[R03] ABORT forbidden staging artifact: %s\n' "${forbidden}" >&2
        exit 25
    fi
done
...
grep -Fq 'Light Grid R02' \
    ...
    printf '[R03] ABORT inherited R02 Light Grid asset is absent\n' >&2
    exit 33
}
...
server {
    ...
    server_name tg-h5-ui-r03.local;
    root ${STAGING_SITE};
    ...
    add_header X-TGB-Staging "R03" always;
...
case "${HTTP_FIRST}" in
    200|302) ;;
    *)
        printf '[R03] ABORT staging first HTTP code: %s\n' "${HTTP_FIRST}" >&2
        exit 27
        ;;
esac
...
[ "${HTTP_FINAL}" = "200" ] || {
    printf '[R03] ABORT staging final HTTP code: %s\n' "${HTTP_FINAL}" >&2
    exit 27
}
case "${FINAL_URL}" in
    "http://tg-h5-ui-r03.local:${LOOPBACK_PORT}/"*) ;;
    *)
        printf '[R03] ABORT staging redirect escaped isolated host\n' >&2
        exit 27
        ;;
esac
grep -qiE '<html|<!doctype' "${STAGING_PRIVATE}/home.html" || {
    printf '[R03] ABORT staging response is not HTML\n' >&2
    exit 27
}
grep -qi '^X-TGB-Staging: R03' "${STAGING_PRIVATE}/home.headers" || {
    printf '[R03] ABORT staging marker header is absent\n' >&2
    exit 27
}
...
if [ "${POST_CODE}" != "405" ]; then
    printf '[R03] ABORT staging write guard code: %s\n' "${POST_CODE}" >&2
    exit 28
fi
...
[ "${MAIN_TABLES}" = "${SOURCE_MAIN_TABLES}" ] || {
    printf '[R03] ABORT main table-count clone mismatch\n' >&2
    exit 34
}
[ "${UC_TABLES}" = "${SOURCE_UC_TABLES}" ] || {
    printf '[R03] ABORT UCenter table-count clone mismatch\n' >&2
    exit 35
}
...
printf '[R03] COMPLETE snapshot_id=%s\n' "${SNAPSHOT_ID}"
printf '[R03] STAGING_LISTENER=127.0.0.1:%s\n' "${LOOPBACK_PORT}"
printf '[R03] HTTP_FIRST=%s HTTP_FINAL=%s REDIRECTS=%s HTTP_POST=%s\n' \
    "${HTTP_FIRST}" "${HTTP_FINAL}" "${REDIRECT_COUNT}" "${POST_CODE}"
printf '[R03] MAIN_TABLES=%s UC_TABLES=%s\n' "${MAIN_TABLES}" "${UC_TABLES}"
printf '[R03] PRODUCTION_CODE_UNCHANGED=PASS\n'
printf '[R03] PRODUCTION_MANIFEST_SHA256=%s\n' "${PRODUCTION_MANIFEST_SHA}"
printf '[R03] ARTIFACT_MANIFEST_SHA256=%s\n' "${ARTIFACT_MANIFEST_SHA}"
