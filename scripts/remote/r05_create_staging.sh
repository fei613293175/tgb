...
SOURCE_STAGING_BASE="/www/staging/tg-h5-ui-r04"
...
STAGE_MAIN_DB="tgb_stage_r05_main"
STAGE_MAIN_USER="tgb_r05_main"
STAGE_UC_DB="tgb_stage_r05_uc"
STAGE_UC_USER="tgb_r05_uc"
...
abort_if_exists() {
    if [ -e "$1" ]; then
        printf '[R05] ABORT existing path: %s\n' "$1" >&2
        exit 20
    fi
}

if [ "$(id -u)" -ne 0 ]; then
    printf '[R05] ABORT: root is required\n' >&2
    exit 21
fi
...
SOURCE_POST_CODE="$(curl -sS -o /dev/null -w '%{http_code}' \
    ...
    "http://127.0.0.1:18084/plugin.php?id=xigua_hb")"
[ "${SOURCE_POST_CODE}" = "405" ] || {
    printf '[R05] ABORT: R04 source staging write guard is %s\n' \
        "${SOURCE_POST_CODE}" >&2
    exit 30
}
...
if ss -ltn | awk '{print $4}' | grep -Eq "(^|:)${LOOPBACK_PORT}$"; then
    printf '[R05] ABORT busy loopback port: %s\n' "${LOOPBACK_PORT}" >&2
    exit 29
fi
...
ROOT_CNF="$(mktemp /tmp/tgb-r05-root.XXXXXX.cnf)"
cleanup() {
    rm -f "${ROOT_CNF}"
}
trap cleanup EXIT
...
log "resolve R04 source database identifiers without printing credentials"
MAIN_SOURCE_DB="$(php -r 'include "/www/staging/tg-h5-ui-r04/site/config/config_global.php"; echo $_config["db"][1]["dbname"];')"
MAIN_TABLE_PREFIX="$(php -r 'include "/www/staging/tg-h5-ui-r04/site/config/config_global.php"; echo $_config["db"][1]["tablepre"];')"
UC_SOURCE_DB="$(php -r 'include "/www/staging/tg-h5-ui-r04/site/uc_server/data/config.inc.php"; echo UC_DBNAME;')"
UC_TABLE_PREFIX="$(php -r 'include "/www/staging/tg-h5-ui-r04/site/uc_server/data/config.inc.php"; echo UC_DBTABLEPRE;')"

[ "${MAIN_SOURCE_DB}" = "tgb_stage_r04_main" ] || {
    printf '[R05] ABORT: unexpected R04 main source database\n' >&2
    exit 31
}
[ "${UC_SOURCE_DB}" = "tgb_stage_r04_uc" ] || {
    printf '[R05] ABORT: unexpected R04 UCenter source database\n' >&2
    exit 32
}

case "${MAIN_SOURCE_DB}:${MAIN_TABLE_PREFIX}:${UC_SOURCE_DB}:${UC_TABLE_PREFIX}" in
    *[!A-Za-z0-9_:]*)
        printf '[R05] ABORT: database identifier contains unsupported characters\n' >&2
        exit 22
        ;;
esac
...
for db_name in "${STAGE_MAIN_DB}" "${STAGE_UC_DB}"; do
    if mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
        ...
        grep -q .; then
        printf '[R05] ABORT existing database: %s\n' "${db_name}" >&2
        exit 23
    fi
done
...
for db_user in "${STAGE_MAIN_USER}" "${STAGE_UC_USER}"; do
    if mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
        ...
        grep -q .; then
        printf '[R05] ABORT existing database user: %s\n' "${db_user}" >&2
        exit 24
    fi
done
...
log "create code snapshot from closed R04 staging"
...
if old in text:
    ...
    raise SystemExit("R04 staging H5 entry is neither local nor expected production URL")
...
for forbidden in \
    ...
    if [ -e "${STAGING_SITE}/${forbidden}" ]; then
        printf '[R05] ABORT forbidden staging artifact: %s\n' "${forbidden}" >&2
        exit 25
    fi
done
...
server {
    ...
    server_name tg-h5-ui-r05.local;
    root ${STAGING_SITE};
    ...
    add_header X-TGB-Staging "R05" always;
...
case "${HTTP_FIRST}" in
    200|302) ;;
    *)
        printf '[R05] ABORT staging first HTTP code: %s\n' "${HTTP_FIRST}" >&2
        exit 27
        ;;
esac
FOLLOW_RESULT="$(curl -sS -L --max-redirs 5 \
    --resolve "tg-h5-ui-r05.local:${LOOPBACK_PORT}:127.0.0.1" \
    -D "${STAGING_PRIVATE}/home-follow.headers" \
    -o "${STAGING_PRIVATE}/home.html" \
    -w '%{http_code}|%{url_effective}|%{num_redirects}' \
    -A 'Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 Chrome/138.0 Mobile Safari/537.36' \
    "http://tg-h5-ui-r05.local:${LOOPBACK_PORT}/plugin.php?id=xigua_hb")"
HTTP_FINAL="${FOLLOW_RESULT%%|*}"
FOLLOW_REST="${FOLLOW_RESULT#*|}"
FINAL_URL="${FOLLOW_REST%%|*}"
REDIRECT_COUNT="${FOLLOW_RESULT##*|}"
[ "${HTTP_FINAL}" = "200" ] || {
    printf '[R05] ABORT staging final HTTP code: %s\n' "${HTTP_FINAL}" >&2
    exit 27
}
case "${FINAL_URL}" in
    "http://tg-h5-ui-r05.local:${LOOPBACK_PORT}/"*) ;;
    *)
        printf '[R05] ABORT staging redirect escaped isolated host\n' >&2
        exit 27
        ;;
esac
grep -qiE '<html|<!doctype' "${STAGING_PRIVATE}/home.html" || {
    printf '[R05] ABORT staging response is not HTML\n' >&2
    exit 27
}
grep -qi '^X-TGB-Staging: R05' "${STAGING_PRIVATE}/home.headers" || {
    printf '[R05] ABORT staging marker header is absent\n' >&2
    exit 27
}
...
if [ "${POST_CODE}" != "405" ]; then
    printf '[R05] ABORT staging write guard code: %s\n' "${POST_CODE}" >&2
    exit 28
fi
...
[ "${MAIN_TABLES}" = "${SOURCE_MAIN_TABLES}" ] || {
    printf '[R05] ABORT main table-count clone mismatch\n' >&2
    exit 34
}
[ "${UC_TABLES}" = "${SOURCE_UC_TABLES}" ] || {
    printf '[R05] ABORT UCenter table-count clone mismatch\n' >&2
    exit 35
}
...
cat >"${STAGING_PRIVATE}/R05_FACTS.txt" <<FACTS
...
FACTS
chmod 600 "${STAGING_PRIVATE}/R05_FACTS.txt"

cat >"${BACKUP_DIR}/RESTORE_README.md" <<RESTORE
# R05 snapshot restore
...
- the closed R04 staging code and databases were the only R05 source baseline;
...
printf '[R05] COMPLETE snapshot_id=%s\n' "${SNAPSHOT_ID}"
printf '[R05] STAGING_LISTENER=127.0.0.1:%s\n' "${LOOPBACK_PORT}"
printf '[R05] HTTP_FIRST=%s HTTP_FINAL=%s REDIRECTS=%s HTTP_POST=%s\n' \
    "${HTTP_FIRST}" "${HTTP_FINAL}" "${REDIRECT_COUNT}" "${POST_CODE}"
printf '[R05] MAIN_TABLES=%s UC_TABLES=%s\n' "${MAIN_TABLES}" "${UC_TABLES}"
printf '[R05] PRODUCTION_CODE_UNCHANGED=PASS\n'
printf '[R05] PRODUCTION_MANIFEST_SHA256=%s\n' "${PRODUCTION_MANIFEST_SHA}"
printf '[R05] ARTIFACT_MANIFEST_SHA256=%s\n' "${ARTIFACT_MANIFEST_SHA}"
