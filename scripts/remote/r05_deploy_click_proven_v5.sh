...
fail() { printf '[R05-V5-DEPLOY] ABORT: %s\n' "$1" >&2; exit 1; }
...
mapfile -t ARCHIVE_FILES < <(tar -tzf "${ARCHIVE_PATH}" | sed '/\/$/d; s#^r05-site-overlay-v5/##' | LC_ALL=C sort)
mapfile -t ALLOWED_FILES < <(printf '%s\n' "${KEEP_FILES[@]}" | LC_ALL=C sort)
[ "${ARCHIVE_FILES[*]}" = "${ALLOWED_FILES[*]}" ] || fail "archive allowlist mismatch"

WORK_DIR="$(mktemp -d "${R05_PRIVATE}/r05-v5-work.XXXXXX")"
...
DETAIL_CSS="${WORK_DIR}/source/plugin/xigua_hb/static/tgb-r05/detail-light-grid-r05.css"
if grep -Eiq 'member-page|redpack-page|feed-page|\.cmt-|comment|like|favorite|mycover|hong_|card-page' "${DETAIL_CSS}"; then
  fail "out-of-scope detail selector remains"
fi

DEPLOY_ID="$(date '+%Y%m%dT%H%M%S%z')"
BACKUP_DIR="${R05_PRIVATE}/change-backups/${DEPLOY_ID}-click-proven-v5"
...
for relative in "${KEEP_FILES[@]}"; do
  cmp "${WORK_DIR}/${relative}" "${R05_SITE}/${relative}" || fail "v5 install mismatch: ${relative}"
done
...
printf '[R05-V5-DEPLOY] PASS\n'
printf '[R05-V5-DEPLOY] DEPLOY_ID=%s ARCHIVE_SHA256=%s\n' "${DEPLOY_ID}" "${ACTUAL_SHA256}"
printf '[R05-V5-DEPLOY] KEEP=12 RESTORED_OR_REMOVED=18 PHP_LINT=PASS HOME=%s POST=%s\n' "${HOME_CODE}" "${POST_CODE}"
printf '[R05-V5-DEPLOY] BACKUP_DIR=%s\n' "${BACKUP_DIR}"
