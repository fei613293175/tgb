...
SOURCE_STAGING_BASE="/www/staging/tg-h5-ui-r07"
SOURCE_STAGING_SITE="${SOURCE_STAGING_BASE}/site"
STAGING_BASE="/www/staging/tg-h5-ui-r08"
STAGING_SITE="${STAGING_BASE}/site"
STAGING_PRIVATE="${STAGING_BASE}/private"
BACKUP_ROOT="/www/backup/tg-h5-ui-r08"
...
STAGE_MAIN_USER="tgb_r08_main"
STAGE_UC_DB="tgb_stage_r08_uc"
STAGE_UC_USER="tgb_r08_uc"
...
abort_if_exists() {
    if [ -e "$1" ]; then
        printf '[R08] ABORT existing path: %s\n' "$1" >&2
        exit 20
    fi
}

if [ "$(id -u)" -ne 0 ]; then
    printf '[R08] ABORT: root is required\n' >&2
    exit 21
fi

test -d "${PRODUCTION_ROOT}/source/plugin"
test -d "${SOURCE_STAGING_SITE}/source/plugin"
test -f "${SOURCE_STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r02/light-grid-r02.css"
test -f "${SOURCE_STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r03/auth-light-grid-r03.css"
test -f "${SOURCE_STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r04/discovery-light-grid-r04.css"
test -x "${PANEL_PYTHON}"
...
SOURCE_POST_CODE="$(curl -sS -o /dev/null -w '%{http_code}' \
    -X POST \
    -H 'Host: tg-h5-ui-r07.local' \
    "http://127.0.0.1:18087/plugin.php?id=xigua_hb")"
[ "${SOURCE_POST_CODE}" = "405" ] || {
    printf '[R08] ABORT: R07 source staging write guard is %s\n' \
        "${SOURCE_POST_CODE}" >&2
    exit 30
}
...
if ss -ltn | awk '{print $4}' | grep -Eq "(^|:)${LOOPBACK_PORT}$"; then
    printf '[R08] ABORT busy loopback port: %s\n' "${LOOPBACK_PORT}" >&2
    exit 29
fi
...
ROOT_CNF="$(mktemp /tmp/tgb-r07-root.XXXXXX.cnf)"
cleanup() {
    rm -f "${ROOT_CNF}"
}
trap cleanup EXIT
...
log "resolve R07 source database identifiers without printing credentials"
MAIN_SOURCE_DB="$(php -r 'include "/www/staging/tg-h5-ui-r07/site/config/config_global.php"; echo $_config["db"][1]["dbname"];')"
MAIN_TABLE_PREFIX="$(php -r 'include "/www/staging/tg-h5-ui-r07/site/config/config_global.php"; echo $_config["db"][1]["tablepre"];')"
UC_SOURCE_DB="$(php -r 'include "/www/staging/tg-h5-ui-r07/site/uc_server/data/config.inc.php"; echo UC_DBNAME;')"
UC_TABLE_PREFIX="$(php -r 'include "/www/staging/tg-h5-ui-r07/site/uc_server/data/config.inc.php"; echo UC_DBTABLEPRE;')"

[ "${MAIN_SOURCE_DB}" = "tgb_stage_r07_main" ] || {
    printf '[R08] ABORT: unexpected R07 main source database\n' >&2
    exit 31
}
[ "${UC_SOURCE_DB}" = "tgb_stage_r07_uc" ] || {
    printf '[R08] ABORT: unexpected R07 UCenter source database\n' >&2
    exit 32
}

case "${MAIN_SOURCE_DB}:${MAIN_TABLE_PREFIX}:${UC_SOURCE_DB}:${UC_TABLE_PREFIX}" in
    *[!A-Za-z0-9_:]*)
        printf '[R08] ABORT: database identifier contains unsupported characters\n' >&2
        exit 22
        ;;
esac
...
for db_name in "${STAGE_MAIN_DB}" "${STAGE_UC_DB}"; do
    if mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
        ...
        grep -q .; then
        printf '[R08] ABORT existing database: %s\n' "${db_name}" >&2
        exit 23
    fi
done
...
for db_user in "${STAGE_MAIN_USER}" "${STAGE_UC_USER}"; do
    if mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
        ...
        grep -q .; then
        printf '[R08] ABORT existing database user: %s\n' "${db_user}" >&2
        exit 24
    fi
done
...
log "create code snapshot from closed R07 staging"
tar -czf "${BACKUP_DIR}/site-code.tar.gz" \
    -C "${SOURCE_STAGING_BASE}" \
    --exclude='site/data/attachment' \
    --exclude='site/data/cache' \
    --exclude='site/data/log' \
    --exclude='site/data/sysdata' \
    --exclude='site/data/template' \
    --exclude='site/source/plugin/xigua_hb/pics' \
    --exclude='site/uc_server/data/avatar' \
    --exclude='site/uc_server/data/cache' \
    --exclude='site/uc_server/data/logs' \
    site
...
for relative in data/attachment uc_server/data/avatar; do
    if [ -e "${SOURCE_STAGING_SITE}/${relative}" ]; then
...
local = "/plugin.php?id=xigua_hb"
if old in text:
    text = text.replace(old, local, 1)
elif local not in text:
    raise SystemExit("R07 staging H5 entry is neither local nor expected production URL")
...
for forbidden in \
    ...
    if [ -e "${STAGING_SITE}/${forbidden}" ]; then
        printf '[R08] ABORT forbidden staging artifact: %s\n' "${forbidden}" >&2
        exit 25
    fi
done
...
grep -Fq 'Light Grid R02' \
    ...
    printf '[R08] ABORT inherited R02 Light Grid asset is absent\n' >&2
    exit 33
}
grep -Fq '#2764ff' \
    "${STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r03/auth-light-grid-r03.css" || {
    printf '[R08] ABORT inherited R03 authentication asset is absent\n' >&2
    exit 36
}
grep -Fq 'Light Grid R04 discovery pages' \
    "${STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r04/discovery-light-grid-r04.css" || {
    printf '[R08] ABORT inherited R04 discovery asset is absent\n' >&2
    exit 37
}
grep -Fq 'Light Grid R05 v5' \
    "${STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r05/detail-light-grid-r05.css" || {
    printf '[R08] ABORT inherited R05 detail asset is absent\n' >&2
    exit 38
}
grep -Fq 'pointer-events: none !important' \
    "${STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r04/discovery-light-grid-r04.css" || {
    printf '[R08] ABORT R05 hidden-category correction is absent\n' >&2
    exit 39
}
grep -Fq 'Light Grid R06 batch A' \
    "${STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r06/account-light-grid-r06.css" || {
    printf '[R08] ABORT inherited R06 account asset is absent\n' >&2
    exit 40
}
grep -Fq 'Light Grid R06: click-proven message list only' \
    "${STAGING_SITE}/source/plugin/xigua_lt/static/tgb-r06/chats-list-light-grid-r06.css" || {
    printf '[R08] ABORT inherited R06 message asset is absent\n' >&2
    exit 41
}
grep -Fq 'Light Grid R07: commission wallet and payout account binding' \
    "${STAGING_SITE}/source/plugin/xigua_hb/static/tgb-r07/finance-light-grid-r07.css" || {
    printf '[R08] ABORT inherited R07 finance asset is absent\n' >&2
    exit 42
}
grep -Fq 'Light Grid R07: standard and super headline purchase surfaces' \
    "${STAGING_SITE}/source/plugin/tb_toutiao/static/tgb-r07/promotion-light-grid-r07.css" || {
    printf '[R08] ABORT inherited R07 promotion asset is absent\n' >&2
    exit 43
}
...
server {
    ...
    server_name tg-h5-ui-r08.local;
    root ${STAGING_SITE};
    ...
    add_header X-TGB-Staging "R08" always;
...
case "${HTTP_FIRST}" in
    200|302) ;;
    *)
        printf '[R08] ABORT staging first HTTP code: %s\n' "${HTTP_FIRST}" >&2
        exit 27
        ;;
esac
FOLLOW_RESULT="$(curl -sS -L --max-redirs 5 \
    --resolve "tg-h5-ui-r08.local:${LOOPBACK_PORT}:127.0.0.1" \
    -D "${STAGING_PRIVATE}/home-follow.headers" \
    -o "${STAGING_PRIVATE}/home.html" \
    -w '%{http_code}|%{url_effective}|%{num_redirects}' \
    -A 'Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 Chrome/138.0 Mobile Safari/537.36' \
    "http://tg-h5-ui-r08.local:${LOOPBACK_PORT}/plugin.php?id=xigua_hb")"
HTTP_FINAL="${FOLLOW_RESULT%%|*}"
FOLLOW_REST="${FOLLOW_RESULT#*|}"
FINAL_URL="${FOLLOW_REST%%|*}"
REDIRECT_COUNT="${FOLLOW_RESULT##*|}"
[ "${HTTP_FINAL}" = "200" ] || {
    printf '[R08] ABORT staging final HTTP code: %s\n' "${HTTP_FINAL}" >&2
    exit 27
}
case "${FINAL_URL}" in
    "http://tg-h5-ui-r08.local:${LOOPBACK_PORT}/"*) ;;
    *)
        printf '[R08] ABORT staging redirect escaped isolated host\n' >&2
        exit 27
        ;;
esac
grep -qiE '<html|<!doctype' "${STAGING_PRIVATE}/home.html" || {
    printf '[R08] ABORT staging response is not HTML\n' >&2
    exit 27
}
grep -qi '^X-TGB-Staging: R08' "${STAGING_PRIVATE}/home.headers" || {
    printf '[R08] ABORT staging marker header is absent\n' >&2
    exit 27
}
...
if [ "${POST_CODE}" != "405" ]; then
    printf '[R08] ABORT staging write guard code: %s\n' "${POST_CODE}" >&2
    exit 28
fi
...
SOURCE_MAIN_TABLES="$(mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
    "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${MAIN_SOURCE_DB}'")"
SOURCE_UC_TABLES="$(mysql --defaults-extra-file="${ROOT_CNF}" -NBe \
    "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${UC_SOURCE_DB}'")"
[ "${MAIN_TABLES}" = "${SOURCE_MAIN_TABLES}" ] || {
    printf '[R08] ABORT main table-count clone mismatch\n' >&2
    exit 34
}
[ "${UC_TABLES}" = "${SOURCE_UC_TABLES}" ] || {
    printf '[R08] ABORT UCenter table-count clone mismatch\n' >&2
    exit 35
}
...
cat >"${STAGING_PRIVATE}/R08_FACTS.txt" <<FACTS
snapshot_id=${SNAPSHOT_ID}
source_release=R07
source_staging_root=${SOURCE_STAGING_SITE}
source_write_guard_http_post=${SOURCE_POST_CODE}
...
r03_auth_legal_inherited=PASS
r04_discovery_inherited=PASS
r05_content_inherited=PASS
r06_account_communication_inherited=PASS
r07_finance_membership_promotion_inherited=PASS
discuz_template_cache_directory=READY
FACTS
chmod 600 "${STAGING_PRIVATE}/R08_FACTS.txt"

cat >"${BACKUP_DIR}/RESTORE_README.md" <<RESTORE
# R08 snapshot restore
...
- the closed R07 staging code and databases were the only R08 source baseline;
...
printf '[R08] COMPLETE snapshot_id=%s\n' "${SNAPSHOT_ID}"
printf '[R08] STAGING_LISTENER=127.0.0.1:%s\n' "${LOOPBACK_PORT}"
printf '[R08] HTTP_FIRST=%s HTTP_FINAL=%s REDIRECTS=%s HTTP_POST=%s\n' \
    "${HTTP_FIRST}" "${HTTP_FINAL}" "${REDIRECT_COUNT}" "${POST_CODE}"
printf '[R08] MAIN_TABLES=%s UC_TABLES=%s\n' "${MAIN_TABLES}" "${UC_TABLES}"
printf '[R08] PRODUCTION_CODE_UNCHANGED=PASS\n'
printf '[R08] PRODUCTION_MANIFEST_SHA256=%s\n' "${PRODUCTION_MANIFEST_SHA}"
printf '[R08] ARTIFACT_MANIFEST_SHA256=%s\n' "${ARTIFACT_MANIFEST_SHA}"
