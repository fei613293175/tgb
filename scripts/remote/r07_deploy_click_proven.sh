...
BASE="/www/staging/tg-h5-ui-r07"
SITE="${BASE}/site"
PRIVATE="${BASE}/private"
HOST="tg-h5-ui-r07.local"
PORT="18087"
ARCHIVE_PATH="${1:-}"
EXPECTED_SHA256="${2:-}"
FILES=(
...
fail() { printf '[R07-DEPLOY] ABORT: %s\n' "$1" >&2; exit 1; }
[ "$(id -u)" -eq 0 ] || fail "root is required"
[ -d "${SITE}/source/plugin" ] || fail "R07 staging is absent"
[ -d "${PRIVATE}" ] || fail "R07 private directory is absent"
...
mapfile -t ARCHIVE_FILES < <(tar -tzf "${ARCHIVE_PATH}" | sed '/\/$/d; s#^r07-site-overlay/##' | LC_ALL=C sort)
mapfile -t ALLOWED_FILES < <(printf '%s\n' "${FILES[@]}" | LC_ALL=C sort)
[ "${ARCHIVE_FILES[*]}" = "${ALLOWED_FILES[*]}" ] || fail "archive allowlist mismatch"

WORK_DIR="$(mktemp -d "${PRIVATE}/r07-overlay.XXXXXX")"
...
printf '[R07-DEPLOY] PASS\n'
printf '[R07-DEPLOY] DEPLOY_ID=%s ARCHIVE_SHA256=%s FILES=12 HOME=%s POST=%s\n' "${DEPLOY_ID}" "${ACTUAL_SHA256}" "${HOME_CODE}" "${POST_CODE}"
printf '[R07-DEPLOY] BACKUP_DIR=%s\n' "${BACKUP_DIR}"
