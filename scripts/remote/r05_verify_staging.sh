...
SOURCE_STAGING_SITE="/www/staging/tg-h5-ui-r04/site"
...
ROOT_CNF="$(mktemp /tmp/tgb-r05-verify.XXXXXX.cnf)"
cleanup() {
    rm -f "${ROOT_CNF}"
}
trap cleanup EXIT
...
unset($_config);
include '/www/staging/tg-h5-ui-r05/site/config/config_global.php';
...
$stageUc = defineValue(
    '/www/staging/tg-h5-ui-r05/site/uc_server/data/config.inc.php',
    'UC_DBNAME'
);
...
printf '[R05-VERIFY] PASS snapshot_id=%s\n' "${SNAPSHOT_ID}"
printf '[R05-VERIFY] STAGING_LISTENER=127.0.0.1:%s\n' "${LOOPBACK_PORT}"
printf '[R05-VERIFY] HTTP_FIRST=%s HTTP_FINAL=%s REDIRECTS=%s HTTP_POST=%s\n' \
    "${FIRST_HTTP_CODE}" "${FINAL_HTTP_CODE}" "${REDIRECT_COUNT}" "${POST_CODE}"
printf '[R05-VERIFY] MAIN_TABLES=%s UC_TABLES=%s\n' \
    "${MAIN_TABLES}" "${UC_TABLES}"
printf '[R05-VERIFY] PRODUCTION_CODE_UNCHANGED=PASS\n'
printf '[R05-VERIFY] PRODUCTION_MANIFEST_SHA256=%s\n' \
    "${PRODUCTION_MANIFEST_SHA}"
printf '[R05-VERIFY] ARTIFACT_MANIFEST_SHA256=%s\n' \
