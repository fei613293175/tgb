...
fail() {
    printf '[R03-CLEANUP] ABORT: %s\n' "$1" >&2
    exit 71
}

[ "$(id -u)" -eq 0 ] || fail "root is required"
[ ! -e "${NGINX_CONFIG}" ] || fail "R03 vhost exists; this is not a partial pre-listener stage"
if ss -ltn | grep -Eq '127\.0\.0\.1:18083([^0-9]|$)'; then
    fail "R03 loopback listener is active"
fi
...
ROOT_CNF="$(mktemp /tmp/tgb-r03-cleanup.XXXXXX.cnf)"
cleanup() {
    rm -f "${ROOT_CNF}"
}
trap cleanup EXIT
...
DROP DATABASE IF EXISTS `tgb_stage_r03_main`;
DROP DATABASE IF EXISTS `tgb_stage_r03_uc`;
DROP USER IF EXISTS 'tgb_r03_main'@'127.0.0.1';
DROP USER IF EXISTS 'tgb_r03_main'@'localhost';
DROP USER IF EXISTS 'tgb_r03_uc'@'127.0.0.1';
DROP USER IF EXISTS 'tgb_r03_uc'@'localhost';
FLUSH PRIVILEGES;
SQL

rm -rf -- "${STAGING_BASE}" "${BACKUP_ROOT}"
printf '[R03-CLEANUP] PASS removed fixed incomplete R03 targets\n'
