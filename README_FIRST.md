...
- 当前版本：`R02`，状态：`IN_PROGRESS`。
- 当前唯一任务：在隔离舞台开发“推广宝”品牌与 H5 公共浅色壳层；不得提前把 R03-R09 页面标记为已改造。
- R01 已开发关版，最新版支付 H5 兼容修复服务器签名包已复制到桌面；GitHub Actions 自动测试已按 D-020 停用，负责人真机反馈异步返回且不阻塞持续开发。
