...
<div class="weui-cells  <!--{if $comment[og_pubid]}-->ogcmt<!--{/if}-->">
    <div class="weui-cell" style="padding: 0px;align-items: end;" style="">
        <div class="weui-cell__hd" style="margin-right: 0px;">
            <img src="{avatar($comment['authorid'], 'big', true)}" alt=""
                 style="width:35px;margin-right:5px;display:block;border-radius: 50%;">
        </div>
        <div class="weui-cell__bd">
            <p class="bigtxt" style="color: #000;font-weight:500;height: 14px;line-height:2px;margin-top:20px;">$comment[author]</p>
            <p class="smalltxt" style="color:#808080;">$comment[crts] ·  IP属地: {$comment[ipaddr]}</p>
        </div>
 ...
 <!--{if !$comment[og_pubid]}-->     <!--<p style="font-size: 0.55rem;color:#fff;border-radius: 15px;padding:2px 8px;background-color: #00A1EB;" onclick="showcomment($comment['authorid'],$comment['cid'])">{lang xigua_hb:huifu}</p>
    ...
    <div class="view-content-comment-text" style="background-color: #fff;border-radius: 10px;">
        {$comment[comment]}
    </div>
</div>
</a>
