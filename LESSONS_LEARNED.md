...
## L-078 点击路由与视觉截图路由必须完全一致

- 现象：历史 `CONTENT-PUBLISH` 证据打开了 `/plugin.php?id=xigua_hb&ac=pub` 分类页，而真实用户从可见发布入口进入的是带 `step=3&catid=31` 的发布表单。
- 风险：page ID 相同或源码同属一个插件并不能证明截图就是用户实际点击到的页面；错误截图会把真正页面的过高顶栏、裁切按钮和空白占位遗漏掉。
- 规则：视觉证据必须保存点击图最终 URL，并与审计中的 requested URL 对比；路径和决定页面形态的查询参数不一致时立即失败，旧证据标记为拒绝而不是复用。

## L-079 DOM 正确仍可能被截图坐标伪造裁切

- 现象：签到公告 DOM 没有横向越界，但一次截图使用了错误的宿主屏幕坐标，右侧被裁掉，看起来像弹层溢出。
- 规则：先用 DOM 几何和命中测试定位真实 UI，再核对 PNG 宽高、裁切原点和视口坐标。只有坐标契约一致的截图可以进入最终目录；错误截图保留到 `rejected-capture/` 并说明否决原因。

## L-080 高视口超过浏览器物理表面会产生截图回卷

- 现象：`430x932` 设备指标高度超过当时浏览器可用物理表面，屏幕级截图在底部重复显示顶部导航，DOM 中实际只有一个 header。
- 规则：设备指标不等于宿主表面足够大。高视口证据必须使用浏览器内容视口的显式 CDP 裁切，断言 header 数量、底部命中元素和 PNG 实际尺寸；屏幕表面截图不得与 CSS 视口坐标混用。

## L-081 生产数据库标识只能在运行时提供

- 现象：收银台生产部署脚本曾把生产数据库标识直接写入仓库，违反交接包不保存数据库标识和凭据的状态声明；在当前站点配置下，该标识还可能与数据库口令相同，风险不能按普通环境名称忽略。
- 规则：生产部署脚本只接受运行时 `EXPECTED_PRODUCTION_DB`，缺失时非零退出；脚本和证据不得打印该值。提交前扫描当前工作树与 Git 历史，历史已暴露时必须登记负责人轮换口令和历史清理决定，不得伪称风险已消失。

## L-082 追加 CSS 不能替代清理高优先级内联旧主题

- 现象：首页头条虽然追加了新版选择器，模板仍保留暖橙阴影、大圆角和标签内联样式，真机看起来仍像旧卡片。
- 规则：对真机否决的控件先删除主容器的旧内联视觉，再用版本 CSS 建立同构布局；同时提升静态资源版本，门禁禁止关键容器重新出现 `style=`。

## L-083 真实设备证据高于历史截图数量

- 现象：39 条历史浏览器映射均有文件，但负责人真机仍出现状态栏叠加、卡片溢出、底栏旧样式和滚动失效。
- 规则：证据映射证明“曾截过图”，不自动证明当前设备体验正确。新的负责人缺陷一旦与 PASS 冲突，必须立即重开状态并禁止旧 PASS 被续接 AI 复用。

## L-084 单一视口同宽不代表小屏同宽

- 现象：390px 下头条卡与普通卡已同宽，但旧的 374px 媒体规则让 360px 下头条左右边距变成 12px，普通卡仍为 16px，产生 8px 宽度差。
- 规则：重复卡片的同构要求必须在最小、主流和较宽三档逐项比较几何值。任何只作用于一个卡片族的媒体缩进都必须删除或与相邻卡片同步。

## L-085 原生零 inset 后 H5 不能再写死状态栏高度

- 现象：基础页面已使用 `env(safe-area-inset-top)`，返修层又以 `.tgb-android-app` 写死 28px 或 36px；浏览器安全区为 0 时看似正常，真机可能再次形成过高顶栏。
- 规则：原生 WebView 保持 edge-to-edge 和四边零 padding；H5 只使用 `env(safe-area-inset-top, 0px)`，禁止固定设备像素模拟状态栏。独立模板也必须带 `viewport-fit=cover`。

## L-086 共享底栏脚本错误会掩盖后续依赖错误

- 现象：九个 `tab*.php` 的可选路由匹配只注释了循环开头，闭合括号仍执行，先触发 `Unexpected token '}'`；个人中心随后又因广告模板依赖 `$` 但页面未加载本地 jQuery 而报错。
- 规则：共享底栏脚本必须先做语法门禁；独立页面引用旧插件脚本时必须显式加载服务器本地依赖。控制台须在部署后重载复验，不能只看 DOM 已渲染。

## L-087 全局数量门禁不能继续引用已被真机否决的旧证据

- 现象：14 个真机缺陷页面已经完成新截图和几何复验，台账也变为 `REDESIGNED_VERIFIED`，但全局证据映射仍指向 R05-R08 历史文件；原门禁只检查39行存在且文件可读，因此会产生形式上的假 PASS。
- 规则：任何负责人否决都会使该 page ID 的历史主证据失效。恢复 PASS 时必须同时换绑本轮最终证据，并在脚本中锁定受影响集合和目标证据，不能只检查数量、状态或文件存在。
- 落地：14行映射已全部指向 `R09_OWNER_DEVICE_REPAIR_FINAL_VERIFY.md`，全局收尾脚本新增逐页断言后重新 PASS。

## L-088 SSH等待超时不等于服务器构建失败或成功

- 现象：首次Android服务器构建的本地SSH等待窗口在14秒超时，但远程Docker和Gradle继续运行，最终在工作区生成了通过测试与Lint的APK；主脚本却没有完成制品归档和签名门禁，因此既不能按编译失败修源码，也不能把半完成目录标为PASS。
- 规则：传输或等待中断后先检查不可变构建目录、容器、测试XML、Lint和APK输出，区分工具链失败与控制通道失败。只有权威构建脚本从头到尾返回0并生成完整artifacts时才可标PASS；重试必须使用新build ID，禁止补写半完成目录冒充完整构建。
- 结果：新构建 `20260727T235000+0800` 完整执行测试、Lint、签名、体积、AAPT、APKSigner和SHA-256门禁后PASS。
