...
## L-068 普通换肤不能被测试流程拖成大型工程

- 现象：R09 为已经完成的 HTML/CSS 换肤重复设计三视口全截图、认证夹具和多轮候选矩阵，开发速度明显低于页面改造本身；一次会话过期还让整批截图实际变成登录页。
- 规则：已确认入口和历史边界视口基线不重复证明。普通 UI 返修默认只跑真实页面 `390x844`、静态业务协议和回滚预检；共享断点或安全区发生变化时才补边界视口。
- 规则：登录态失效必须通过最终 URL 和页面标题立即判失败，不能以背景色或截图文件存在冒充真实页面通过。
- 结果：R09 改为一次认证快速回归、三张代表页人工视觉抽查和五页线上冒烟后完成生产部署。
...
## L-070 H5 站点不能用桌面 UA 做范围结论

- 现象：搜索 `_blank` 新标签最初没有继承预期移动 UA，页面只显示桌面壳层；若据此
  判断页面不存在或设计异常，会再次造成范围和视觉误判。
- 规则：父页、重定向页和每个新标签都必须核对 Android H5 UA 与目标视口；子标签
  未继承时重新施加 UA 并重载。桌面 UA 只能作为无效探针，不得写入 PASS 证据。
- 结果：搜索子页重载后真实渲染 20 个项目，无横向溢出；公开登录链、防诈骗、设置
  和个人中心也全部按同一 UA 口径完成收尾。

## L-071 已经上线的超范围 UI 也必须实际恢复

- 现象：会员协议、帮助和关于曾因直接 URL 可打开被 R03/R09 纳入候选，虽然真实
  Android H5 个人中心和设置中心没有入口，但生产文件已经被换肤。
- 规则：将页面标为 `OUT_OF_VISUAL_SCOPE` 还不够；必须从可信的上线前备份恢复文件，
  校验前后 SHA-256，建立新的生产备份和可执行回滚，再从后续候选构建中排除。
## L-072 先建隧道再打开浏览器目标页

- 现象：隔离站 SSH 隧道尚未监听时先打开目标页，内置浏览器生成 `data:` 错误文档；之后即使隧道和服务器已经健康，Browser Use 安全策略仍拒绝操作该错误标签。
- 规则：浏览器验收必须按“服务器状态 -> 桥状态 -> SSH 监听 -> curl 只读探针 -> 新浏览器标签 -> UA/视口 -> 目标页”的顺序执行。连接失败的错误标签直接结束会话，不在其上尝试 CDP、脚本或其他浏览器绕行。
- 发布边界：静态门禁和网络探针不能替代视觉结果。没有改后截图与几何 JSON 时，隔离部署必须回滚，交接文档明确记录准确续点。

## L-073 私有备份元数据不能复制回公开模板目录

- 现象：R09 早期回滚使用 `cp -a` 把私有备份目录的 `0700 root:root` 元数据带入插件 `template/touch`，PHP-FPM 的 `www` 用户无法读取模板，页面返回空内容。
- 规则：回滚只能逐文件 `install -o www -g www -m 0644`，公开模板目录必须显式保持 `0755 www:www`；部署结束同时执行目录权限断言和 `runuser -u www -- test -r`。
- 落地：隔离和生产的成员/私信部署脚本均采用逐文件恢复、PHP-FPM 可读门禁与私有备份，禁止恢复整个私有目录树。

## L-074 Discuz 移动模板缓存会被错误 UA 重编译

- 现象：收银台首个 Android H5 标签正常渲染；新标签未先设置移动 UA 时，Discuz 把同名缓存重新编译为桌面 `template/main.htm`，随后因旧桌面模板引用不存在的 `template/header.htm` 返回 System Error。
- 规则：每个新标签必须先设置 Android H5 UA，再导航到目标页；部署触屏模板后只删除精确的 `data/template/1_tb_pay_main.tpl.php` 生成缓存，让正确 UA 重建。桌面错误页不得作为 H5 缺陷或视觉证据。

## L-075 清理脚本不能假设旧插件表都有 id 字段

- 现象：收银台夹具首次清理在导出审计行时查询不存在的 `id` 列，门禁中止，桥已关闭但合成订单仍待清理。
- 规则：旧 Discuz 插件表的清理和审计必须使用已经验证存在的业务键，并采用确定排序；脚本失败后修复同一夹具再执行，不能绕过为手工删除。
- 结果：改为按 `dateLine/dateline + orderid` 导出后，1 条合成会员订单删除、0 条支付订单确认、调试日志恢复、POST 405。

## L-076 浏览器 innerHeight 不证明截图像素完整

- 现象：几何报告为 `430x932`，普通截图 API 却导出 `430x882`，固定底部支付按钮在图片证据中被截断。
- 规则：每张视觉证据必须读取 PNG 实际宽高；不一致时使用受控设备画布精确捕获，重新覆盖截图并再次人工查看。DOM 几何 PASS 和截图文件存在都不能替代像素尺寸门禁。
- 结果：收银台三张最终 PNG 分别精确为 `360x800`、`390x844`、`430x932`。

## L-077 长任务必须用产出而不是思考时长证明推进

- 现象：范围复杂或验证链较长时，重复读取同一文档和反复讨论方案容易形成没有新证据的原地循环。
- 规则：每 30 分钟及阶段边界记录上一项实质产出和下一项可执行动作；连续两次没有新制品、测试、部署或已解决事实时立即更换方法。异步负责人反馈不成为等待理由。
